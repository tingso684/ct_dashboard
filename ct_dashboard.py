...
if not csv_files:
    ...
    with layout_tab1:
        ...
        with st.container():
            ...
            with layout_col2:
                selected_gas = st.selectbox("Select a gas", options=gas_type, index=1)
        ...
        with st.container():
            ...
            df_top10 = df_top10.loc[df_top10['rank']<=10,:]
            df_top10['contents'] = df_top10['country'] + df_top10['emissions_quantity'].apply(lambda x: " ({:,.0f}M)".format(x / 1_000_000))
            df_top10 = df_top10.pivot_table(index=['sector','emissions_total'], columns='rank',values='contents', aggfunc='first')
        ...
        with st.container():
            ...
            filtered_df_top500 = filtered_df_top500.set_index(['rank','iso3_country','sector','subsector','asset_type','asset_name','asset_id'])            
            
            st.dataframe(filtered_df_top500, use_container_width=True, height=st_layout['height_row']-80)
